<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="Adminpage.aspx.cs" Inherits="OnlineFlightBookingWeb.Adminpage" %>
<asp:Content ID="adminBody" ContentPlaceHolderID="masterBody" runat="server">
    <div aria-readonly="True">
        <asp:GridView ID="idFlightView1" runat="server" AutoGenerateColumns="False" 
            DataKeyNames="flightId" AutoGenerateDeleteButton="True"
            AutoGenerateEditButton="True" onrowcancelingedit="FlightView_RowCancelingEdit"   
            onrowdeleting="FlightView_RowDeleting" onrowediting="FlightView_RowEditing"   
            onrowupdating="FlightView_RowUpdating" showfooter="true">
            <Columns>
                 <asp:TemplateField HeaderText="FLIGHT ID" runat="server">
                    <ItemTemplate>  
                        <asp:Label ID="LabelFlightId" runat="server" Text='<%# Bind("flightId") %>'></asp:Label>  
                    </ItemTemplate>
                     <FooterTemplate>
                        <asp:LinkButton ID ="btnInsert" Text ="Insert" runat ="server" OnClick ="btnInsert_Click">
                         </asp:LinkButton>
                    </FooterTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="FLIGHT NAME" runat="server">
                    <EditItemTemplate>  
                        <asp:TextBox ID="TxtFlightName" runat="server" Text='<%# Bind("flightName") %>'></asp:TextBox> 
                    </EditItemTemplate>
                    <ItemTemplate>  
                        <asp:Label ID="LabelFlightName" runat="server" Text='<%# Bind("flightName") %>'></asp:Label>  
                    </ItemTemplate>
                     <FooterTemplate>
                        <asp:TextBox ID="txtInsertFlightName" runat="server" />
                    </FooterTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="FLIGHT NUMBER" runat="server">
                    <EditItemTemplate>  
                        <asp:TextBox ID="TxtFlightNumber" runat="server" Text='<%# Bind("flightNumber") %>'></asp:TextBox> 
                    </EditItemTemplate>
                    <ItemTemplate>  
                        <asp:Label ID="LabelFlightNumber" runat="server" Text='<%# Bind("flightNumber") %>'></asp:Label>  
                    </ItemTemplate>
                    <FooterTemplate>
                        <asp:TextBox ID="txtInsertFlightNumber" runat="server" />
                    </FooterTemplate>
                </asp:TemplateField>
            </Columns>
        </asp:GridView>
    </div>
</asp:Content>
